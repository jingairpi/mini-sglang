...
- **Scheduler Worker**: The core worker process. In a multi-GPU setup, there is one Scheduler Worker for each GPU (referred to as a **TP Rank**). It manages the computation and resource allocation for that specific GPU (or CPU in CPU mode).
