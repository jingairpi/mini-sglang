...
- **Scheduler Worker**: The core worker process. It manages computation and resource allocation for one tensor-parallel rank on the selected execution device.

### Data Flow

The components communicate using **ZeroMQ (ZMQ)** for control messages and `torch.distributed` for tensor-parallel coordination.
...
4. **Scheduler (Rank 0)** broadcasts the request to all other Schedulers (if using tensor parallelism).
...
- `minisgl.attention`: Provides attention backend interfaces and implementations for CPU and CUDA execution. They are called by `AttentionLayer` and use metadata stored in `Context`.
...
- `minisgl.engine`: Implements `Engine` class, which is a TP worker on a single process. It manages the model, context, KVCache, attention backend, and optional CUDA graph replay.
...
- `minisgl.kernel`: Implements tensor helpers using CPU PyTorch paths and CUDA kernels supported by `tvm-ffi` for Python binding and JIT compilation.
